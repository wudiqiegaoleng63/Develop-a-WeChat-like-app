...
func (m *messageService) UploadAvatar(c *gin.Context) (string, int) {
	...
	mFprm := c.Request.MultipartForm

	for key, _ := range mFprm.File {
        ...
        ext := filepath.Ext(fileHeader.Filename) //比如 cat.jpg 得到 .jpg，dog.png 得到 .png

        zlog.Info(ext)
        loaclFileName := config.GetConfig().StaticAvatarPath + "/" + fileHeader.Filename

        out, err := os.Create(loaclFileName)
        if err != nil {
            zlog.Error(err.Error())
            return constants.SYSTEM_ERROR, -1
        }
